...
- **Moteur principal depuis le 25/09/2026 (soir) : OpenAI `gpt-4.1-mini`, avec la clé de NOVA** (`OPENAI_API_KEY` sur peg-int et peg-prod, compte payant de 200 000 tokens/min). Décision de Nova : « go comme NOVA ». Groq ne sert plus que de **secours automatique** : si OpenAI est en panne ou saturé, le chat bascule sur Groq sans rien casser. Détail : `CLAUDE.md` de peg_strapi, section « moteur OpenAI ». Ancien moteur Groq : env **`GROQ_MODEL`**, défaut **`openai/gpt-oss-120b`** (mise à jour 23/09/2026). ⚠️ `llama-3.3-70b-versatile` a été **retiré du palier gratuit Groq le 16/08/2026** : toute l'IA PEG est restée muette 38 jours sans alerte. `GROQ_MODEL` est posé sur peg-int et peg-prod ; l'Express le lit aussi. Modèles raisonnants → `reasoning_effort: 'low'` (géré par `groqChat()`). Boucle agentique côté backend, **max 5 itérations**.
...
- Panier : `personalizationStatus(item)` → **pop-up « Ajoutez votre logo »** (`cart/PersonalizeDialog.tsx`, réutilise `WizardShowForm` : fichier réellement téléversé, URL dans `answer.data`), ouverte **d'elle-même** en arrivant du chat (`state.openPersonalization`) ou via le bouton de l'article ; enchaîne sur l'article suivant. ⚠️ Ne pas renvoyer vers la fiche en mode modification : son récapitulatif propose « Ajouter au panier » = **doublon** de l'article (bug historique).
...
- Palier gratuit Groq : 8 000 tokens/min par modèle ET plafond journalier (le 120b a été bloqué 10 min d'affilée le 25/09). Une offre ≈ 6 700 tokens → environ une offre par minute et par modèle. **Résolu le soir même** : moteur principal OpenAI (voir plus haut), Groq en secours. Mesuré sur int : offre 3,7 s, suivi de commande 5,5 s, factures 3,9 s, salutation 1,9 s.
...
- **Réponse lue phrase par phrase au fil du flux** (`POST /chatbot/voice/speak`, voix **française** Henri) : `SentenceSplitter` coupe sur `. ! ?` + espace et chaque ligne (jamais dans le texte d'un lien, ni après « 1. » / « M. ») ; `speakableText` lit le **nom** d'un lien, jamais l'adresse, sans emphase/puces/emojis. Lecture **programmée à l'avance** sur l'horloge du contexte audio, silence d'encodeur sauté (leçon NOVA : enchaîner sur `onended` hachait la voix).
- **La voix ne parle pas anglais et n'annonce que le TTC** (demande Nova 25/09, le texte affiché ne change pas) :
  - Pas Rémy (voix « multilingue » de NOVA) : mesuré, elle passait à l'anglais (« Casquette trucker » → « Casket Trucker »). Le serveur écarte toute voix « Multilingual » (`ttsVoices`).
  - `spokenPrices` : « 89 € HT (106,80 € TTC) » → « 106 euros 80 TTC » ; HT seul converti par `toTTC` (règle du panier) ; « /mois » → « par mois ». L'agent étiquette chaque montant HT ou TTC (`TOOL_GUIDANCE`) : un montant sans mention serait lu tel quel.
  - `frenchWords` : couleurs anglaises du catalogue dites en français (BLACK → noir, HEATHER GREY → gris chiné), mots en MAJUSCULES lus comme des mots (sigles BAT, PDF, TTC, XXL… préservés).
...
- **Lentille de verre, visible SEULEMENT sous le doigt** (demandes Nova 25/09 : « effet glace, morphing », puis « plus réaliste », « qui n'apparaît que quand je touche la barre ») : au repos, l'onglet actif garde sa pastille. Doigt posé → la lentille naît sur l'onglet actif et coule jusqu'à l'onglet touché (bords animés à part `GLASS_LEAD` / `GLASS_TAIL` : elle s'étire puis se rétracte), doigt levé → elle fond dans la pastille (`GLASS_LINGER_MS`). Verre incolore (fond de la barre `--peg-dock-solid` pour masquer les onglets d'origine, voile blanc, arête lumineuse par masque), **réfraction** = copie des onglets dans la lentille (`.peg-dock-glass-lens`, recalée par `lensX`, grossie autour de son centre, dispersion rouge/bleu selon la vitesse). Tout fonctionne sous Safari (aucun filtre SVG, que WebKit ignore en `backdrop-filter`). Défilement de la barre ou appui long → elle fond aussitôt ; animations réduites → jamais affichée. ⚠️ `syncEdges` ne compte que `:scope > .peg-dock-item` (pas les copies de la lentille).
...
- ⚠️ **Fiche produit (admin)** : le sélecteur ne liste que les tailles/couleurs de la catégorie du produit, MAIS toute valeur déjà rattachée hors de cette catégorie y est aussi affichée, marquée « · hors catégorie », avec une note (`Forms/attachedOptions.ts`, tests `attachedOptions.test.ts`). Avant le 25/09/2026, elle était invisible mais restait rattachée et proposée au client (cartes de visite Icart : « vertical » invisible dans l'admin, visible chez le client). **Ne pas les retirer d'office** : 91 tailles / 43 couleurs dans ce cas en prod, souvent voulues (S→4XL de t-shirts dont la catégorie n'a pas ces tailles).
