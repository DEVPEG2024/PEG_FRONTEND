...
- **Cartes au style des tableaux de bord** (demande Nova 25/09, page par page, téléphone seulement) :
  - **Projets** : `ProjectCardMobile.tsx` (affichage seul, `ProjectItem` calcule tout et bascule sous md) — carte vitrée 22px, photo produit pleine largeur **jamais rognée** sur fond « studio » clair (`mix-blend-mode: multiply` : le blanc des photos s'y fond), statut/priorité posés sur la photo, montant en grand. Mêmes infos et libellés que l'ordinateur ; ordinateur et tablette vérifiés identiques au pixel. Styles « CARTES PROJET » de `_mobile.css`.
