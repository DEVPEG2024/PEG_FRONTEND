...
const Button = forwardRef<HTMLButtonElement, ButtonProps>((props, ref) => {
  ...
  const twoToneColor = () => {
    const btn = {
      bgColor: active
        ...
        : `bg-${buttonColor}-50/20 dark:bg-${buttonColor}-500/20`,
      textColor: `text-${buttonColor}-${buttonColorLevel} dark:text-${buttonColor}-50`,
      hoverColor: active
        ? ''
        : `hover:bg-${buttonColor}-100/30 dark:hover:bg-${buttonColor}-500/30`,
      activeColor: `active:bg-${buttonColor}-200/40 dark:active:bg-${buttonColor}-500/40`,
    };
    return getBtnColor(btn);
  };
  ...
  const getBtnColor = ({
    ...
    return `${bgColor} ${
      disabled || loading ? disabledClass : hoverColor + ' ' + activeColor + ' cursor-pointer'
    } ${textColor}`;
  };
