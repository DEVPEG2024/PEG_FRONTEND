...
function ModalEditInvoice({editInvoiceDialog, selectedInvoice, setEditInvoiceDialog, setSelectedInvoice, updateInvoice, dispatch, loading} : {
  ...
  const [vatEnabled, setVatEnabled] = useState<boolean>(selectedInvoice.vatAmount > 0);
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="flex flex-row gap-2">
            <div className="flex flex-col gap-2 w-1/2">
              ...
              <Select
                placeholder="Statut"
                options={formData.paymentState === 'fulfilled' ? stateData.filter(({value}) => value === 'fulfilled') : stateData}
              ...
              <Select
                ...
                onChange={(e: any) => {
                  const newValues = {
                    ...
                    state: e.value === 'fulfilled' ? 'fulfilled' : formData.state
                  }
            ...
            {formData.paymentState === 'fulfilled' && (
              <div className="flex flex-col gap-2 w-1/2">
                <p className="text-sm text-gray-200 mb-2 mt-4">Date de paiement</p>
                <DatePicker
                  placeholder="Date de paiement"
                  value={dayjs(formData.paymentDate).toDate()}
                  inputPrefix={<HiOutlineCalendar className="text-lg" />}
                  onChange={(date: Date | null) => {
                    setFormData({ ...formData, paymentDate: dayjs(date).toDate() });
                  }}
                  inputFormat="DD/MM/YYYY"
                />
              </div>
