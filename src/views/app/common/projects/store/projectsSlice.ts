...
import { apiCreateTransaction } from '@/services/TransactionsServices';
  ...
  async (data: PayProducer): Promise<Project> => {
    ...
    const { updateProject }: { updateProject: Project } = await unwrapData(
      apiUpdateProject({
        ...
        producerPaidPrice:
          data.project.producerPaidPrice + data.transaction.amount,
      })
    );
    return updateProject;
  }
...
export const { setNewProjectDialog } = projectListSlice.actions;
