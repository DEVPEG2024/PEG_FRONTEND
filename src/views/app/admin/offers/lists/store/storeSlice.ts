...
import { apiGetOffers, apiPutStatusOffer, apiDeleteOffer, apiUpdateOffer } from '@/services/OfferServices'
...
export const getOffers = createAsyncThunk(
    ...
    async (data: GetProductListRequest) => {
        const response = await apiGetOffers(data.page, data.pageSize, data.searchTerm)
...
const licencieSlice = createSlice({
    ...
    reducers: {
        ...
        setOffer: (state, action) => {
            state.offer = state.offers.find((offer) => offer._id === action.payload) ?? null
        },
        ...
        setEditProduct: (state, action) => {
            const offer = state.offers.find((offer) => offer._id === action.payload._id);
            if (offer) {
                    state.offer = offer;
            }
        },
