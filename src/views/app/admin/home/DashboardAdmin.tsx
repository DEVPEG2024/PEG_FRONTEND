...
function AreaChart({ data }: { data: { label: string; ca: number; marge: number }[] }) {
  ...
  const caP = data.map((d, i) => `${x(i)},${y(d.ca)}`).join(' '); const mgP = data.map((d, i) => `${x(i)},${y(d.marge)}`).join(' ')
  const caA = `M${pL},${pT + pH} L${caP.split(' ').join(' L')} L${x(data.length - 1)},${pT + pH} Z`; const mgA = `M${pL},${pT + pH} L${mgP.split(' ').join(' L')} L${x(data.length - 1)},${pT + pH} Z`
  const ticks = Array.from({ length: 5 }, (_, i) => Math.round((mx / 4) * i))
  return <div><div className="mb-3 flex items-center gap-5 text-xs"><span className="flex items-center gap-2 text-cyan-400"><span className="inline-block h-2 w-6 rounded-full bg-gradient-to-r from-cyan-400 to-cyan-600" />CA</span><span className="flex items-center gap-2 text-emerald-400"><span className="inline-block h-2 w-6 rounded-full bg-gradient-to-r from-emerald-400 to-emerald-600" />Marge</span></div><svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 220 }} onMouseLeave={() => setHi(null)}><defs><linearGradient id="caG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#22d3ee" stopOpacity="0.25" /><stop offset="100%" stopColor="#22d3ee" stopOpacity="0" /></linearGradient><linearGradient id="mgG" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#34d399" stopOpacity="0.2" /><stop offset="100%" stopColor="#34d399" stopOpacity="0" /></linearGradient></defs>{ticks.map((v, i) => <g key={i}><line x1={pL} x2={W - pR} y1={y(v)} y2={y(v)} stroke="rgba(255,255,255,0.06)" /><text x={pL - 8} y={y(v) + 4} textAnchor="end" fontSize="10" fill="rgba(255,255,255,0.3)">{v >= 1000 ? `${Math.round(v / 1000)}k` : v}</text></g>)}<path d={caA} fill="url(#caG)" /><path d={mgA} fill="url(#mgG)" /><polyline points={caP} fill="none" stroke="#22d3ee" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ filter: 'drop-shadow(0 0 8px rgba(34,211,238,0.4))' }} /><polyline points={mgP} fill="none" stroke="#34d399" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ filter: 'drop-shadow(0 0 8px rgba(52,211,153,0.4))' }} />{data.map((d, i) => { const cx = x(i); const zw = pW / Math.max(data.length, 1); const hov = hi === i; const ttW = 110; const ttX = Math.max(pL, Math.min(cx - ttW / 2, W - pR - ttW)); return <g key={i}><rect x={cx - zw / 2} y={pT} width={zw} height={pH} fill="transparent" onMouseEnter={() => setHi(i)} />{hov && <line x1={cx} x2={cx} y1={pT} y2={pT + pH} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 3" />}<circle cx={cx} cy={y(d.ca)} r={hov ? 6 : 4} fill="#22d3ee" stroke="#0f172a" strokeWidth="2" /><circle cx={cx} cy={y(d.marge)} r={hov ? 6 : 4} fill="#34d399" stroke="#0f172a" strokeWidth="2" /><text x={cx} y={H - 12} textAnchor="middle" fontSize="11" fill={hov ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.45)'}>{d.label}</text>{hov && <g><rect x={ttX} y={pT - 2} width={ttW} height={36} rx={8} fill="rgba(15,23,42,0.92)" stroke="rgba(255,255,255,0.12)" strokeWidth="1" /><text x={ttX + ttW / 2} y={pT + 13} textAnchor="middle" fontSize="10" fill="#22d3ee">CA: {eur(d.ca)}</text><text x={ttX + ttW / 2} y={pT + 27} textAnchor="middle" fontSize="10" fill="#34d399">Marge: {eur(d.marge)}</text></g>}</g> })}</svg></div>
}
            ...
            {!dataReady ? <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Array.from({ length: 4 }).map((_, i) => <KPISkeleton key={i} />)}</div> : (
              <AnimatedSection className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <motion.div variants={fadeInUp} custom={2}>
                  <GlassCard glow={margePct >= 30 ? 'emerald' : margePct >= 15 ? 'amber' : 'rose'}><div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${vc[margePct >= 30 ? 'success' : margePct >= 15 ? 'warning' : 'danger'].gradient} opacity-80`} /><div className="relative p-5"><div className="text-[11px] font-medium uppercase tracking-wider text-white/40">Marge brute</div><div className="flex items-center gap-3 mt-2"><ProgressRing pct={margePct} color={margePct >= 30 ? '#34d399' : margePct >= 15 ? '#fbbf24' : '#fb7185'} /><div><div className="text-2xl font-black text-white">{margePct}%</div><div className="text-xs text-white/45">{eur(margeBrute)}</div></div></div></div></GlassCard>
                </motion.div>
              </AnimatedSection>
            )}
          </div>
