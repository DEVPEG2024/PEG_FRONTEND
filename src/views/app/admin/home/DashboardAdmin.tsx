...
function MeshBackground() {
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      ...
      />
      {/* Mesh blobs */}
      <div className="absolute -top-40 -right-40 w-[700px] h-[700px] rounded-full opacity-[0.035]" style={{ background: 'radial-gradient(circle, #22d3ee, transparent 70%)' }} />
      <div className="absolute top-1/2 -left-40 w-[600px] h-[600px] rounded-full opacity-[0.025]" style={{ background: 'radial-gradient(circle, #8b5cf6, transparent 70%)' }} />
      <div className="absolute -bottom-40 right-1/4 w-[500px] h-[500px] rounded-full opacity-[0.02]" style={{ background: 'radial-gradient(circle, #ec4899, transparent 70%)' }} />
    </div>
  )
}
...
function HeroIllustration() {
  return (
    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 1200 300" preserveAspectRatio="xMidYMid slice" fill="none">
      ...
      {[{ cx: 200, cy: 180, r: 4 }, { cx: 500, cy: 120, r: 5 }, { cx: 800, cy: 80, r: 4 }, { cx: 350, cy: 200, r: 3 }, { cx: 650, cy: 150, r: 3 }, { cx: 950, cy: 110, r: 4 }, { cx: 1100, cy: 60, r: 3 }].map((n, i) => (
        <g key={i}>
...
function PulseDot({ color = 'rose' }: { color?: 'rose' | 'amber' | 'sky' }) {
  const c = { rose: 'bg-rose-500', amber: 'bg-amber-500', sky: 'bg-sky-500' }
  const r = { rose: 'bg-rose-400', amber: 'bg-amber-400', sky: 'bg-sky-400' }
  return (
    <span className="relative flex h-2.5 w-2.5 shrink-0">
      <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${r[color]} opacity-50`} />
...
function MedalBadge({ rank, color }: { rank: number; color: 'emerald' | 'violet' }) {
  if (rank <= 3) {
    const medals = [{ fill: '#FFD700', label: '1' }, { fill: '#C0C0C0', label: '2' }, { fill: '#CD7F32', label: '3' }]
...
function DeltaBadge({ current, previous }: { current: number; previous: number }) {
  ...
  const up = pct > 0
  return (
    <span className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${up ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'}`}>
      {up ? <HiOutlineArrowSmUp className="w-3 h-3" /> : <HiOutlineArrowSmDown className="w-3 h-3" />}
      {up ? '+' : ''}{pct}%
    </span>
  )
}
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
      ...
      <circle
        ...
        style={{ filter: `drop-shadow(0 0 4px ${color}60)`, transition: 'stroke-dashoffset 1s ease-out' }}
      />
    </svg>
  )
}
...
/* ═══════════════════════════════════════════════════════════
   SECTION HEADER with deco
   ═══════════════════════════════════════════════════════════ */

function SectionDeco({ variant }: { variant: 'finances' | 'operations' | 'entities' | 'default' }) {
  const c = { finances: '#22d3ee', operations: '#f59e0b', entities: '#8b5cf6', default: '#64748b' }[variant]
  return (
...
/* ═══════════════════════════════════════════════════════════
   #6 — GLASSMORPHISM CARD (enhanced glow)
   ═══════════════════════════════════════════════════════════ */
...
function GlassCard({ children, className = '', onClick, glow }: { children: React.ReactNode; className?: string; onClick?: () => void; glow?: 'cyan' | 'emerald' | 'amber' | 'rose' | 'violet' | 'sky' }) {
  ...
  // #6: Enhanced corner glow colors
  const cornerGlow: Record<string, string> = {
    cyan: 'radial-gradient(ellipse at top left, rgba(34,211,238,0.08) 0%, transparent 50%)',
    emerald: 'radial-gradient(ellipse at top left, rgba(52,211,153,0.08) 0%, transparent 50%)',
    amber: 'radial-gradient(ellipse at top left, rgba(251,191,36,0.06) 0%, transparent 50%)',
    rose: 'radial-gradient(ellipse at top left, rgba(251,113,133,0.06) 0%, transparent 50%)',
    violet: 'radial-gradient(ellipse at top left, rgba(139,92,246,0.08) 0%, transparent 50%)',
    sky: 'radial-gradient(ellipse at top left, rgba(56,189,248,0.07) 0%, transparent 50%)',
  }
  ...
  return (
    ...
    >
      ...
      {glow && <div className="absolute inset-0 pointer-events-none" style={{ background: cornerGlow[glow] }} />}
      {children}
    </div>
  )
}
...
function KPI({ title, value, subtitle, icon, variant = 'default', onClick, sparkData, delta }: {
  title: string; value: string; subtitle?: string; icon?: React.ReactNode; variant?: KpiVariant; onClick?: () => void; sparkData?: number[]; delta?: React.ReactNode
}) {
  const cfg = variantConfig[variant]
  return (
    <GlassCard onClick={onClick} glow={variant === 'success' ? 'emerald' : variant === 'warning' ? 'amber' : variant === 'danger' ? 'rose' : 'cyan'}>
      <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${cfg.gradient} opacity-80`} />
      <div className="relative p-5">
        ...
        {onClick && <div className={`mt-3 text-[11px] font-medium ${cfg.text} opacity-70`}>Voir d\u00E9tails \u2192</div>}
      </div>
    </GlassCard>
  )
}
...
/* ═══════════════════════════════════════════════════════════
   #1 — KPI HERO CARD (CA total — big, 2 cols)
   ═══════════════════════════════════════════════════════════ */

function KPIHero({ ca, caLastMonth, sparkData, onClick }: { ca: number; caLastMonth: number; sparkData: number[]; onClick: () => void }) {
  return (
    <GlassCard onClick={onClick} glow="cyan" className="lg:col-span-2">
      <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-cyan-400 via-blue-500 to-violet-500 opacity-90" />
      {/* Background sparkline large */}
      <div className="absolute bottom-0 left-0 right-0 h-24 opacity-20 pointer-events-none">
        <Sparkline data={sparkData} color="#22d3ee" height={96} />
      </div>
      <div className="relative p-6 md:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-xs font-semibold uppercase tracking-widest text-cyan-400/60 flex items-center gap-2">
              <HiOutlineLightningBolt className="w-3.5 h-3.5" />
              Chiffre d'affaires total
            </div>
            <div className="mt-3 text-4xl md:text-5xl font-black text-white tracking-tight">
              <AnimatedValue value={ca} format={eur} />
            </div>
            <div className="flex items-center gap-3 mt-3">
              <DeltaBadge current={ca} previous={caLastMonth} />
              <span className="text-xs text-white/35">vs mois pr\u00E9c\u00E9dent</span>
            </div>
          </div>
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-600/10 ring-1 ring-cyan-500/25 shrink-0 text-cyan-400">
            <HiOutlineCurrencyEuro className="w-7 h-7" />
          </div>
        </div>
      </div>
    </GlassCard>
...
function DonutChart({ data, size = 160, thickness = 22, centerLabel, centerValue }: { data: { label: string; value: number }[]; size?: number; thickness?: number; centerLabel?: string; centerValue?: string }) {
  const total = data.reduce((a, d) => a + d.value, 0) || 1
  const radius = (size - thickness) / 2
  const circ = 2 * Math.PI * radius
  const segments = useMemo(() => {
    let cum = 0
    return data.map((d, i) => { const pct = d.value / total; const dash = pct * circ; const gap = circ - dash; const rot = (cum / total) * 360 - 90; cum += d.value; return { dash, gap, rot, color: DONUT_COLORS[i % DONUT_COLORS.length], label: d.label, value: d.value, pct } })
  }, [data, total, circ])
    ...
    <div className="flex flex-col sm:flex-row items-center gap-4">
      <div className="relative shrink-0" style={{ width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          {segments.map((s, i) => (
            <circle key={i} cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={s.color} strokeWidth={thickness} strokeDasharray={`${s.dash} ${s.gap}`} strokeLinecap="round" transform={`rotate(${s.rot} ${size / 2} ${size / 2})`} style={{ filter: `drop-shadow(0 0 6px ${s.color}40)` }} />
          ))}
        </svg>
      ...
      <div className="space-y-1.5 flex-1 min-w-0 w-full">
        {segments.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-white/55 truncate flex-1">{s.label}</span>
            <span className="text-[11px] font-semibold text-white/75">{s.value}</span>
            <span className="text-[10px] text-white/30 w-8 text-right">{Math.round(s.pct * 100)}%</span>
          </div>
        ))}
      </div>
    </div>
...
function AreaChart({ data }: { data: { label: string; ca: number; marge: number }[] }) {
  ...
  const W = 700, H = 220, padL = 50, padR = 20, padT = 20, padB = 35
  const maxVal = useMemo(() => { let m = 1; for (const p of data) m = Math.max(m, p.ca, p.marge); return m * 1.1 }, [data])
  const plotW = W - padL - padR, plotH = H - padT - padB
  const div = Math.max(data.length - 1, 1)
  const x = (i: number) => padL + (i / div) * plotW
  const y = (v: number) => padT + (1 - v / maxVal) * plotH
  const caP = data.map((d, i) => `${x(i)},${y(d.ca)}`).join(' ')
  const mgP = data.map((d, i) => `${x(i)},${y(d.marge)}`).join(' ')
  const caA = `M${padL},${padT + plotH} L${caP.split(' ').join(' L')} L${x(data.length - 1)},${padT + plotH} Z`
  const mgA = `M${padL},${padT + plotH} L${mgP.split(' ').join(' L')} L${x(data.length - 1)},${padT + plotH} Z`
  const tickVals = Array.from({ length: 5 }, (_, i) => Math.round((maxVal / 4) * i))
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 220 }} onMouseLeave={() => setHoverIdx(null)}>
        <defs>
          <linearGradient id="caGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#22d3ee" stopOpacity="0.25" /><stop offset="100%" stopColor="#22d3ee" stopOpacity="0" /></linearGradient>
          <linearGradient id="mgGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#34d399" stopOpacity="0.2" /><stop offset="100%" stopColor="#34d399" stopOpacity="0" /></linearGradient>
        </defs>
        {tickVals.map((v, i) => (<g key={i}><line x1={padL} x2={W - padR} y1={y(v)} y2={y(v)} stroke="rgba(255,255,255,0.06)" /><text x={padL - 8} y={y(v) + 4} textAnchor="end" fontSize="10" fill="rgba(255,255,255,0.3)">{v >= 1000 ? `${Math.round(v / 1000)}k` : v}</text></g>))}
        <path d={caA} fill="url(#caGrad)" /><path d={mgA} fill="url(#mgGrad)" />
        ...
        {data.map((d, i) => { const cx = x(i); const zw = plotW / Math.max(data.length, 1); const h = hoverIdx === i; return (<g key={i}><rect x={cx - zw / 2} y={padT} width={zw} height={plotH} fill="transparent" onMouseEnter={() => setHoverIdx(i)} />{h && <line x1={cx} x2={cx} y1={padT} y2={padT + plotH} stroke="rgba(255,255,255,0.15)" strokeDasharray="4 3" />}<circle cx={cx} cy={y(d.ca)} r={h ? 6 : 4} fill="#22d3ee" stroke="#0f172a" strokeWidth="2" /><circle cx={cx} cy={y(d.marge)} r={h ? 6 : 4} fill="#34d399" stroke="#0f172a" strokeWidth="2" /><text x={cx} y={H - 12} textAnchor="middle" fontSize="11" fill={h ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.45)'}>{d.label}</text>{h && <g><rect x={cx - 55} y={padT - 2} width={110} height={36} rx={8} fill="rgba(15,23,42,0.92)" stroke="rgba(255,255,255,0.12)" strokeWidth="1" /><text x={cx} y={padT + 13} textAnchor="middle" fontSize="10" fill="#22d3ee">CA: {eur(d.ca)}</text><text x={cx} y={padT + 27} textAnchor="middle" fontSize="10" fill="#34d399">Marge: {eur(d.marge)}</text></g>}</g>) })}
      </svg>
    </div>
  )
}
...
function HorizontalBars({ items, onClick }: { items: { label: string; value: number }[]; onClick?: (l: string) => void }) {
  ...
  return (
    <div className="space-y-3">
      {items.map((it, idx) => { const pct = Math.round((it.value / max) * 100); return (
        <div key={it.label}>
          <div className="flex items-center justify-between mb-1">
            <button type="button" onClick={() => onClick?.(it.label)} className={['text-xs text-white/65 truncate', onClick ? 'hover:text-white transition' : ''].join(' ')}>{it.label}</button>
            <span className="text-xs font-semibold text-white/70">{it.value}</span>
          </div>
          <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, ease: 'easeOut', delay: idx * 0.05 }} className="h-full rounded-full" style={{ background: `linear-gradient(90deg, ${BAR_COLORS[idx % BAR_COLORS.length]}, ${BAR_COLORS[idx % BAR_COLORS.length]}88)`, boxShadow: `0 0 10px ${BAR_COLORS[idx % BAR_COLORS.length]}40` }} />
          </div>
        </div>
      ) })}
    </div>
...
const TODO_KEY = 'peg:dashboardTodos'
...
function TodoListWidget() {
  ...
  useEffect(() => { const r = localStorage.getItem(TODO_KEY); if (r) { try { setTodos(JSON.parse(r)) } catch {} } }, [])
  const persist = useCallback((n: TodoItem[]) => { setTodos(n); localStorage.setItem(TODO_KEY, JSON.stringify(n)) }, [])
    ...
    <GlassCard className="p-5" glow="violet">
      ...
      <div className="relative flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500/20 to-purple-600/10 ring-1 ring-violet-500/20 text-violet-400"><HiOutlineClipboardList className="w-4 h-4" /></div>
          <div><h3 className="text-sm font-bold text-white">Pense-b\u00EAte</h3><p className="text-[10px] text-white/35">{pending.length} en cours \u00B7 {done.length} termin\u00E9(s)</p></div>
        </div>
      ...
      <div className="relative space-y-1.5 max-h-[280px] overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,0.1) transparent' }}>
        ...
        {[...pending, ...done].map(t => (
          <motion.div key={t.id} layout initial={{ opacity: 0, x: -10 }} animate={{ opacity: t.done ? 0.4 : 1, x: 0 }} className="group flex items-center gap-3 rounded-xl px-3 py-2 transition hover:bg-white/[0.03]">
            <button onClick={() => toggleTodo(t.id)} className={['flex h-5 w-5 shrink-0 items-center justify-center rounded-md border transition', t.done ? 'bg-violet-500/30 border-violet-500/40 text-violet-300' : 'border-white/15 hover:border-violet-500/40'].join(' ')}>
...
const CAL_KEY = 'peg:calendarEvents'
...
function CalendarMiniWidget() {
  ...
  useEffect(() => { const r = localStorage.getItem(CAL_KEY); if (r) { try { setEvents(JSON.parse(r)) } catch {} } }, [])
  const todayEv = events.filter(e => dayjs(e.start).isSame(today, 'day')).sort((a, b) => dayjs(a.start).valueOf() - dayjs(b.start).valueOf())
  const ws = today.startOf('isoWeek'), we = today.endOf('isoWeek')
  const weekEv = events.filter(e => { const s = dayjs(e.start); return s.isAfter(ws) && s.isBefore(we) && !s.isSame(today, 'day') }).sort((a, b) => dayjs(a.start).valueOf() - dayjs(b.start).valueOf()).slice(0, 5)
  ...
  return (
    <GlassCard className="p-5" glow="sky">
      ...
      <div className="relative flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-sky-500/20 to-blue-600/10 ring-1 ring-sky-500/20 text-sky-400"><HiOutlineCalendar className="w-4 h-4" /></div>
          <div><h3 className="text-sm font-bold text-white capitalize">{vm.format('MMMM YYYY')}</h3><p className="text-[10px] text-white/35">{todayEv.length} aujourd'hui</p></div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => setMOff(o => o - 1)} className="text-white/30 hover:text-white/60 transition p-1"><HiOutlineChevronLeft className="w-3.5 h-3.5" /></button>
          <button onClick={() => setMOff(0)} className="text-[10px] text-sky-400/50 hover:text-sky-300 transition px-1">Auj.</button>
          <button onClick={() => setMOff(o => o + 1)} className="text-white/30 hover:text-white/60 transition p-1"><HiOutlineChevronRight className="w-3.5 h-3.5" /></button>
      ...
      <div className="relative grid grid-cols-7 gap-1 mb-4">
        ...
        {Array.from({ length: dim }).map((_, i) => { const day = i + 1; const isT = mOff === 0 && day === today.date(); const cd = vm.date(day); const he = events.some(e => dayjs(e.start).isSame(cd, 'day')); return (
          <div key={day} className={['text-[10px] text-center py-1 rounded-md relative', isT ? 'bg-sky-500 text-white font-bold shadow-lg shadow-sky-500/30' : 'text-white/40', he && !isT ? 'text-white/70 font-semibold' : ''].join(' ')}>
            {day}{he && !isT && <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-sky-400" />}
          </div>
        ) })}
      </div>
      {mOff === 0 && todayEv.length > 0 && <div className="relative space-y-1.5 mb-3"><div className="text-[10px] uppercase tracking-wider text-white/30 mb-1">Aujourd'hui</div>{todayEv.map(ev => { const c = CAT_COLORS[ev.category] ?? CAT_COLORS.autre; return <div key={ev.id} className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 ${c.bg}`}><span className={`w-1.5 h-1.5 rounded-full shrink-0 ${c.dot}`} /><span className={`text-xs font-medium truncate ${c.text}`}>{ev.title}</span><span className="text-[10px] text-white/30 ml-auto shrink-0">{dayjs(ev.start).format('HH:mm')}</span></div> })}</div>}
      {mOff === 0 && weekEv.length > 0 && <div className="relative space-y-1.5"><div className="text-[10px] uppercase tracking-wider text-white/30 mb-1">Cette semaine</div>{weekEv.map(ev => <div key={ev.id} className="flex items-center gap-2 text-xs text-white/50"><span className="text-[10px] text-white/25 w-10 shrink-0">{dayjs(ev.start).format('ddd')}</span><span className="truncate">{ev.title}</span><span className="text-[10px] text-white/20 ml-auto shrink-0">{dayjs(ev.start).format('HH:mm')}</span></div>)}</div>}
    </GlassCard>
  )
}

/* ═══════════════════════════════════════════════════════════
   #7 — ACTIVITY FEED (enriched with icons + relative time)
   ═══════════════════════════════════════════════════════════ */

function ActivityFeed({ items }: { items: { left: string; right: string; sub?: string; type?: 'invoice' | 'project' }[] }) {
  return (
    <div className="space-y-2">
      {items.length === 0 && <div className="text-xs text-white/30 text-center py-4">Aucune activit\u00E9</div>}
      {items.map((item, i) => (
        <div key={i} className="flex items-start gap-3 group">
          <div className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-lg shrink-0 ${item.type === 'invoice' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-cyan-500/10 text-cyan-400'}`}>
            {item.type === 'invoice' ? <HiOutlineDocumentDuplicate className="w-3.5 h-3.5" /> : <HiOutlineCube className="w-3.5 h-3.5" />}
          </div>
          <div className="flex-1 min-w-0">
...
export default function DashboardAdmin() {
  ...
  const fetchDashboard = async () => {
    try { setLoading(true); setError(null); const res = await apiGetDashboardSuperAdminInformations(); const data = (res as any)?.data?.data ?? (res as any)?.data ?? null; if (!data) throw new Error('R\u00E9ponse GraphQL vide'); setGql(data); setLastUpdated(new Date()) } catch (e: any) { setError(e?.message ?? 'Erreur') } finally { setLoading(false) }
  }
  ...
  // #4 — Live timestamp
  useEffect(() => {
    const update = () => { if (lastUpdated) setLastUpdatedText(dayjs(lastUpdated).fromNow()) }
    update()
  ...
  const invoicePaid = useMemo(() => { const fi = invoices.reduce((a: number, x: any) => { const ps = (x?.paymentState ?? '').toString().toLowerCase(); const st = (x?.state ?? '').toString().toLowerCase(); return a + ((ps === 'fulfilled' || st === 'fulfilled' || ps.includes('paid') || ps === 'paye' || st.includes('paid')) ? (Number(x?.totalAmount) || 0) : 0) }, 0); const fp = projects.reduce((a: number, p: any) => a + (Array.isArray(p?.invoices) && p.invoices.length > 0 ? 0 : (Number(p?.paidPrice) || 0)), 0); return fi + fp }, [invoices, projects])
  ...
  // #5 — Revenue last month for delta
  const revenue6m = useMemo(() => {
    ...
    const by = new Map<string, { ca: number; costs: number }>(); months.forEach(k => by.set(k, { ca: 0, costs: 0 }))
  ...
  const caSparkData = revenue6m.map(d => d.ca); const margeSparkData = revenue6m.map(d => d.marge)
        ...
        <div className="relative z-10 space-y-6 pb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-white/[0.08] aspect-[4/1] md:aspect-[5/1]">
            ...
            <div className="absolute left-4 md:left-6 bottom-4 md:bottom-6">
              ...
              <p className="text-white/50 text-xs md:text-sm mt-1">
                {attentionCount > 0 ? `${attentionCount} point${attentionCount > 1 ? 's' : ''} d'attention` : 'Tout est en ordre'} \u2014 {dayjs().format('dddd D MMMM')}
              </p>
            </div>
            <div className="absolute right-4 md:right-5 top-4 md:top-5 flex items-center gap-2">
              ...
              <button onClick={() => setRefreshTick(t => t + 1)} className="bg-white/10 backdrop-blur-md border border-white/15 text-white/80 px-3 py-1.5 rounded-xl hover:bg-white/15 transition text-xs flex items-center gap-1.5">
          ...
          <div>
            ...
            {!dataReady ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">{Array.from({ length: 5 }).map((_, i) => <KPISkeleton key={i} />)}</div> : (
              <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                <motion.div variants={fadeInUp} custom={0} className="col-span-2">
                  <KPIHero ca={invoiceTotal} caLastMonth={caLastMonth} sparkData={caSparkData} onClick={() => navigate('/admin/invoices')} />
                </motion.div>
                <motion.div variants={fadeInUp} custom={1}>
                  <KPI title="Encaiss\u00E9" value={eur(invoicePaid)} icon={<HiOutlineCheckCircle className="w-5 h-5" />} variant="success" onClick={() => navigate('/admin/invoices')} delta={<DeltaBadge current={invoicePaid} previous={caLastMonth * 0.7} />} />
                </motion.div>
                ...
                <motion.div variants={fadeInUp} custom={3}>
                  {/* #8 — Marge with progress ring */}
                  <GlassCard glow={margePct >= 30 ? 'emerald' : margePct >= 15 ? 'amber' : 'rose'}>
                    <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${variantConfig[margePct >= 30 ? 'success' : margePct >= 15 ? 'warning' : 'danger'].gradient} opacity-80`} />
                    <div className="relative p-5">
                      <div className="text-[11px] font-medium uppercase tracking-wider text-white/40">Marge brute</div>
                      <div className="flex items-center gap-3 mt-2">
                        <ProgressRing pct={margePct} color={margePct >= 30 ? '#34d399' : margePct >= 15 ? '#fbbf24' : '#fb7185'} />
                        <div>
                          <div className="text-2xl font-black text-white">{margePct}%</div>
                          <div className="text-xs text-white/45">{eur(margeBrute)}</div>
                        </div>
                      </div>
                    </div>
                  </GlassCard>
          ...
          <div>
            ...
            {!dataReady ? <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">{Array.from({ length: 5 }).map((_, i) => <KPISkeleton key={i} />)}</div> : (
              <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {[
                  { t: 'Projets', v: String(projectsTotal), i: <HiOutlineCube className="w-5 h-5" />, o: () => navigate('/common/projects') },
                  { t: 'Projets \u00E0 risque', v: String(atRiskProjects), i: <HiOutlineExclamation className="w-5 h-5" />, vr: (atRiskProjects > 0 ? 'danger' : 'default') as KpiVariant, o: () => navigate('/common/projects') },
                  { t: 'Commandes', v: String(orderItemsTotal), i: <HiOutlineShoppingCart className="w-5 h-5" />, o: () => navigate('/admin/order-items') },
                  { t: 'D\u00E9lai moyen', v: `${avgDeliveryDays}j`, i: <HiOutlineClock className="w-5 h-5" />, s: 'Livraison' },
                  { t: 'Tickets ouverts', v: String(openTickets), i: <HiOutlineTicket className="w-5 h-5" />, vr: (openTickets > 0 ? 'warning' : 'default') as KpiVariant, o: () => navigate('/support') },
                ].map((k, idx) => (
                  <motion.div key={k.t} variants={fadeInUp} custom={idx}>
                    <KPI title={k.t} value={k.v} icon={k.i} variant={k.vr} onClick={k.o} subtitle={k.s} />
                  </motion.div>
                ))}
              </motion.div>
            )}
          ...
          <div>
            ...
            {!dataReady ? <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Array.from({ length: 4 }).map((_, i) => <KPISkeleton key={i} />)}</div> : (
              <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { t: 'Clients', v: String(customersTotal), i: <HiOutlineUserGroup className="w-5 h-5" />, o: () => navigate('/admin/customers/list') },
                  { t: 'Producteurs', v: String(producersTotal), i: <HiOutlineOfficeBuilding className="w-5 h-5" />, o: () => navigate('/admin/producers/list') },
                  { t: 'Transactions', v: String(transactions.length), i: <HiOutlineSwitchHorizontal className="w-5 h-5" />, s: eur(totalCosts) },
                  { t: 'Factures retard', v: String(overdueInvoices), i: <HiOutlineDocumentText className="w-5 h-5" />, vr: (overdueInvoices > 0 ? 'danger' : 'default') as KpiVariant, o: () => navigate('/admin/invoices') },
                ].map((k, idx) => (
                  <motion.div key={k.t} variants={fadeInUp} custom={idx}>
                    <KPI title={k.t} value={k.v} icon={k.i} variant={k.vr} onClick={k.o} subtitle={k.s} />
                  </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <motion.div variants={scaleIn} className="lg:col-span-2">
              <GlassCard className="p-5 md:p-6" glow="cyan">
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-cyan-500 to-blue-600 opacity-80" />
            ...
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="violet">
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-violet-500 to-purple-600 opacity-80" />
                <SectionHeader title="Tickets" subtitle={`${ticketsTotal} au total`} />
                {!dataReady ? <ChartSkeleton height="h-[180px]" /> : ticketsByState.length > 0 ? <DonutChart data={ticketsByState} centerValue={String(ticketsTotal)} centerLabel="total" /> : <div className="text-center py-6"><EmptyTicket /><div className="text-xs text-white/25">Aucun ticket</div></div>}
              </GlassCard>
            </motion.div>
          </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="cyan">
                ...
                <SectionHeader title="Pipeline" subtitle="Par statut" right={<button onClick={() => navigate('/common/projects')} className="text-[11px] text-cyan-400/60 hover:text-cyan-300 transition">Ouvrir \u2192</button>} />
                {!dataReady ? <ChartSkeleton height="h-[160px]" /> : <HorizontalBars items={pipeline} onClick={l => navigate(`/common/projects?state=${encodeURIComponent(l)}`)} />}
              </GlassCard>
            </motion.div>
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="amber">
                ...
                <SectionHeader title="Commandes" subtitle={`${orderItemsTotal} au total`} />
                {!dataReady ? <ChartSkeleton height="h-[180px]" /> : ordersByState.length > 0 ? <DonutChart data={ordersByState} centerValue={String(orderItemsTotal)} centerLabel="total" /> : <div className="text-center py-6"><EmptyTicket /><div className="text-xs text-white/25">Aucune commande</div></div>}
              </GlassCard>
            </motion.div>
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="rose">
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-rose-500 to-red-500 opacity-80" />
                <SectionHeader title="Alertes" subtitle="Points d'attention" />
                {!dataReady ? <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10" />)}</div> : (
                  <div className="space-y-2">
                    {overdueInvoices > 0 ? <AlertCard variant="danger" pulse>{overdueInvoices} facture(s) en retard</AlertCard> : <AlertCard variant="success">Aucune facture en retard</AlertCard>}
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="emerald">
                ...
                {!dataReady ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-8" />)}</div> : (
                  <div className="space-y-3">{topClients.map((c, i) => <div key={i} className="flex items-center gap-3"><MedalBadge rank={i + 1} color="emerald" /><div className="flex-1 min-w-0"><div className="text-sm text-white/75 truncate">{c.name}</div></div><div className="text-sm font-semibold text-emerald-400/80 shrink-0">{eur(c.revenue)}</div></div>)}</div>
                )}
              </GlassCard>
            </motion.div>
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="violet">
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-violet-500 to-purple-500 opacity-80" />
                <SectionHeader title="Top producteurs" subtitle="Par projets" right={<button onClick={() => navigate('/admin/producers/list')} className="text-[11px] text-violet-400/60 hover:text-violet-300 transition">Voir tous \u2192</button>} />
                {!dataReady ? <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-10" />)}</div> : (
                  <div className="space-y-3">{topProducers.map((p, i) => <div key={i} className="flex items-center gap-3"><MedalBadge rank={i + 1} color="violet" /><div className="flex-1 min-w-0"><div className="text-sm text-white/75 truncate">{p.name}</div><div className="text-[10px] text-white/30">{p.projects} projet(s)</div></div><div className="text-sm font-semibold text-violet-400/80 shrink-0">{p.revenue ? eur(p.revenue) : '\u2014'}</div></div>)}</div>
                )}
              </GlassCard>
            </motion.div>
            <motion.div variants={scaleIn}>
              <GlassCard className="p-5 md:p-6" glow="sky">
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-sky-500 to-blue-500 opacity-80" />
                <SectionHeader title="Activit\u00E9 r\u00E9cente" subtitle="Projets & factures" />
                {!dataReady ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-8" />)}</div> : <ActivityFeed items={activity} />}
              </GlassCard>
            </motion.div>
          </motion.div>
