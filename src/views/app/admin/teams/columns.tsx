...
export const useColumns = (fetchTeams: () => void, handleEditTeam: (team: IUser) => void) => {
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => (
        <div className="flex justify-end items-center gap-2">
          ...
          />
          <Button onClick={() => handleEditTeam(row.original)} size="sm" variant="twoTone">
            <HiPencil size={20} />
          </Button>
          <Button onClick={() => handleDeleteTeam(row.original._id)} size="sm" variant="twoTone"> 
            <HiTrash size={20} />
          </Button>
        </div>
      )
