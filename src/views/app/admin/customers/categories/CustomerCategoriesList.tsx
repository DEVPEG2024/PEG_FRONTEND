...
import { HiOutlineSearch, HiPlus, HiPencil, HiTrash, HiTag } from 'react-icons/hi';
...
const CustomerCategoriesList = () => {
  ...
  const { total, customerCategories, loading, customerCategory } = useAppSelector((state) => state.customerCategories.data);
  ...
  const filtered = customerCategories.filter((c) =>
  ...
  return (
    <Container style={{ fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', paddingTop: '28px', paddingBottom: '24px', flexWrap: 'wrap' }}>
        <div>
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '11px', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '4px' }}>Clients</p>
          <h2 style={{ color: '#fff', fontSize: '22px', fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>
            Catégories clients <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '16px', fontWeight: 500 }}>({total})</span>
          </h2>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ background: 'linear-gradient(160deg, #16263d 0%, #0f1c2e 100%)', borderRadius: '16px', padding: '64px 24px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.07)' }}>
          <HiTag size={48} style={{ color: 'rgba(255,255,255,0.1)', margin: '0 auto 14px', display: 'block' }} />
          ...
          {filtered.map((cat) => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
                <Btn onClick={() => { dispatch(setCustomerCategory(cat)); setIsOpenEdit(true); }} icon={<HiPencil size={13} />} hoverBg="rgba(47,111,237,0.15)" hoverColor="#6b9eff" hoverBorder="rgba(47,111,237,0.4)" title="Modifier" />
                <Btn onClick={() => { dispatch(setCustomerCategory(cat)); setIsOpenDelete(true); }} icon={<HiTrash size={13} />} hoverBg="rgba(239,68,68,0.12)" hoverColor="#f87171" hoverBorder="rgba(239,68,68,0.3)" title="Supprimer" />
              </div>
            </div>
          ))}
        </div>
      ...
      {customerCategory && isOpenEdit && <ModalEditCustomerCategory mode="edit" title={t('cat.editCategory')} isOpen={isOpenEdit} handleCloseModal={handleCloseModal} />}
      {isOpen && <ModalEditCustomerCategory mode="add" title={t('cat.addCategory')} isOpen={isOpen} handleCloseModal={handleCloseModal} />}
      {customerCategory && isOpenDelete && <ModalDeleteCustomerCategory title={t('cat.deleteCategory')} isOpen={isOpenDelete} handleCloseModal={handleCloseModal} />}
    </Container>
  );
};
