...
import { apiGetOrderItems } from '@/services/OrderServices';
...
function ModalNewInvoice() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    dispatch(
      createInvoice({
        invoice: {
          ...(formData as unknown as InvoiceOld),
  ...
  const fetchCustomers = async () => {
    ...
    const customersList: Customer[] = customers_connection.nodes || [];
  ...
  const fetchOrderItems = async () => {
    const {orderItems_connection} : {orderItems_connection: GetOrderItemsResponse} = await unwrapData(apiGetOrderItems());
    const orderItemsList: OrderItem[] = orderItems_connection.nodes || [];
    const orderItems = orderItemsList.map((orderItem: OrderItem) => ({
      value: orderItem.documentId || '',
      label: orderItem. || '',
    }));
    setProjects(orderItems);
  };
