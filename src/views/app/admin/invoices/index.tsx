...
import reducer, { deleteInvoice, getInvoices, setEditInvoiceDialog, setNewInvoiceDialog, setSelectedInvoice, useAppSelector } from "./store";
...
const Invoices = () => {
  ...
  useEffect(() => {
    dispatch(getInvoices({ page: currentPage, pageSize: pageSize, searchTerm: searchTerm }))
  ...
  const columns = useColumns(handleUpdateInvoice, handleDeleteInvoice, handlePrintInvoice);
  ...
  return (
    <Container>
      ...
      />
        ...
        {invoice && (
          <ModalPrintInvoice invoice={invoice as Invoice} isOpen={modalPrintInvoice} onClose={() => setModalPrintInvoice(false)} />
        )}
    </Container>
  );
}
