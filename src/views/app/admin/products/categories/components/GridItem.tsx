...
const GridItem = ({ data, handleDeleteProject }: { data: CategoryProduct, handleDeleteProject: (id: string) => void }) => {
  const {
    title,
    image,
    totalProducts
  ...
  return (
    <Card bodyClass=" bg-gray-900 rounded-lg project-card justify-center items-center" onClick={() => navigate(`/admin/store/categories/${data._id}`)}>
        ...
        <div className="flex justify-center mt-4">  
          <Button variant="twoTone" onClick={() => handleDeleteProject(data._id)}>Supprimer</Button>
        </div>
    </Card>
  );
};
