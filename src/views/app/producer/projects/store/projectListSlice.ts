...
import { apiChangeTaskStatus, apiCreateTask, apiGetInvoicesProject,  apiCreateInvoice, apiDeleteInvoice, apiUpdateInvoice, apiGetProjectsCustomer, apiGetProjectsProducer } from '@/services/ProjectServices'
...
type Query = {
   page: number, pageSize: number, searchTerm: string, producerId: string 
}
...
export const getList = createAsyncThunk(
    ...
    async (data: GetProjectListRequest) => {
        const response = await apiGetProjectsProducer(data.page, data.pageSize, data.searchTerm, data.producerId);
        ...
        setEditTaskSelected: (state, action) => {
            ...
            if (state.selectedProject) {
              state.selectedProject.tasks = state.selectedProject.tasks.map(
                (task: ITask) => task._id === action.payload._id ? action.payload : task
              );
        ...
        builder.addCase(changeTaskStatus.fulfilled, (state, action) => {
            ...
            state.projectList = state.projectList.map(project => {
                if (project._id === action.payload.projectId) {
                    return {
                        ...project,
                        tasks: project.tasks.map(task => task._id === action.payload.task._id ? action.payload.task : task)
                    }
                }
                return project
            })
