...
function PaymentContent({cart, createOrderAndClearCart} : {cart: CartItem[], createOrderAndClearCart: (paymentInformations: PaymentInformations) => Promise<void>}) {
  const [isSubmitting, setSubmitting] = useState<boolean>(false);

  const validateCart = async () => {
    setSubmitting(true);
    const {paymentValidated, paymentInformations} : {paymentValidated: boolean, paymentInformations: PaymentInformations} = await validatePayment();
    if (paymentValidated) {
